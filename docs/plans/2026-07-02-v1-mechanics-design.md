...
- **ИИ-конкурент равный игроку**: играет по тем же правилам через те же
  публичные API (симметричный агент), простые детерминированные эвристики.
...
- Новая фаза тика **между караванами и рынком труда**:
  ...
  завод мог нанять в тот же тик. Фиксируется в CLAUDE.md (M7), не молча.
...
- Новые ключевые решения: симметрия агентов (ИИ только через публичные API);
  ...
  в `gameplay._migrate_save`); enum Good — только добавление в конец.
